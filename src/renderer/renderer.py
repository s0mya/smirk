...
from .util import batch_orth_proj, face_vertices, vertex_normals
...
class Renderer(nn.Module):
    def __init__(self, render_full_head=False, obj_filename="assets/head_template.obj"):
        ...
        # Fix the paths
        if not obj_filename.startswith("/"):
            obj_filename = os.path.join(DIRNAME, "../..", obj_filename)
        ...
        flame_masks = pickle.load(
            open(
                os.path.join(DIRNAME, "../..", "assets/FLAME_masks/FLAME_masks.pkl"),
                "rb",
            ),
            encoding="latin1",
        )
